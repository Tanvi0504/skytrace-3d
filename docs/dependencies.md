...
| Node.js/npm | 20-24 | Builds the React/Three.js frontend | `npm --prefix frontend ci` or frontend Docker image | Node runtime; no GPU requirement |
...
| FFmpeg | System package or portable executable | Reliable video codec support | `apt-get install ffmpeg` in Docker; platform package manager natively; Windows portable builds can be placed at `tools/ffmpeg/ffmpeg.exe` | Must be on `PATH`; the native readiness check also detects the standard Winget portable install and `tools/ffmpeg/ffmpeg.exe` |
| COLMAP | System package or portable release | Sparse/dense photogrammetry reconstruction | `apt-get install colmap` in Docker; platform package manager natively; Windows portable builds can be extracted to `tools/colmap` | Must be on `PATH` or available as `tools/colmap/colmap.exe` / `tools/colmap/COLMAP.bat`; CPU works, CUDA is optional |
